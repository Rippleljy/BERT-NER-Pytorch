...
#从初始文本获取test文件（不带标注）
# todo: ljy
def buildTestFileFromTxt(data_dir):
    input_file = os.path.join(data_dir, "predict.txt")
    out_file = os.path.join(data_dir, "test.char.bmes")
    with open(input_file, 'r', encoding="utf-8-sig") as f:
        with open(str(out_file), 'w', encoding="utf-8-sig") as fw:
            for line in f:
                for word in line:
...
class CnerProcessor(DataProcessor):
    ...
    def get_predict_examples(self, data_dir, set_type):
        """See base class."""
        outfile = buildTestFileFromTxt(data_dir)
        return self._create_examples(self._read_text(outfile), set_type)

    def get_labels(self):
        """See base class."""
        return ["O", "I-ORG", "I-DATE", "B-ORG", "I-LOC", "I-PER", "<START>", "<END>", "B-DATE", "B-PER", "B-LOC"]
